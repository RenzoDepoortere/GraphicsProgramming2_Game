...
// GEOMETRY SHADER *
//******************
void CreateVertex(inout TriangleStream<GS_DATA> triStream, float3 pos, float3 normal)	// Deleted: float2 texCoord
{
...
void SpikeGenerator(triangle VS_DATA vertices[3], inout TriangleStream<GS_DATA> triStream)
{
	...
	float3 vertex_1 = vertices[1].Position - vertices[0].Position;
	float3 vertex_2 = vertices[2].Position - vertices[1].Position;
	float3 vertex_3 = vertices[0].Position - vertices[2].Position;
	float3 vertex_4 = basePoint - vertices[0].Position;
	float3 vertex_5 = basePoint - vertices[1].Position;
	float3 vertex_6 = basePoint - vertices[2].Position;
	
        //FACE 1
        float3 faceNormal_1 = normalize(cross(vertex_4, vertex_1));
        CreateVertex(triStream, vertices[0].Position, faceNormal_1);
		CreateVertex(triStream, vertices[1].Position, faceNormal_1);
        CreateVertex(triStream, basePoint, faceNormal_1);
        ...
        //FACE 2
        float3 faceNormal_2 = normalize(cross(vertex_5, vertex_2));
		CreateVertex(triStream, vertices[1].Position, faceNormal_2);
		CreateVertex(triStream, vertices[2].Position, faceNormal_2);
        CreateVertex(triStream, basePoint, faceNormal_2);

		triStream.RestartStrip();
		
        //Face 3
        float3 faceNormal_3 = normalize(cross(vertex_6, vertex_3));
		CreateVertex(triStream, vertices[2].Position, faceNormal_3);
		CreateVertex(triStream, vertices[0].Position, faceNormal_3);
        CreateVertex(triStream, basePoint, faceNormal_3);
