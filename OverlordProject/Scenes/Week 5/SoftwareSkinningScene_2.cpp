...
void SoftwareSkinningScene_2::Update()
{
	...
	// Transform vertices
	bool useFirstBoneTransform{};
	XMVECTOR originalPosition{};
	XMVECTOR transformedPosition{};
	...
	for (size_t idx{}; idx < m_SkinnedVertices.size(); ++idx)
	{
		...
		// Use firstBoneTransform
		useFirstBoneTransform = idx < 24;
		if (useFirstBoneTransform)
		{
			transformedPosition = XMVector3TransformCoord(originalPosition, firstBoneTransform);
		}
		// Use secondBoneTransform
		else
		{
			transformedPosition = XMVector3TransformCoord(originalPosition, secondBoneTransform);
		}

		// Store new position
		XMStoreFloat3(&newPosition, transformedPosition);
