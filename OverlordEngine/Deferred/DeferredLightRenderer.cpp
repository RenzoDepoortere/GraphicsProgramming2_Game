...
void DeferredLightRenderer::DirectionalLightPass(const SceneContext& sceneContext, ID3D11ShaderResourceView* const gbufferSRVs[]) const
{
	...
	if (light.isEnabled)
	{
		...
		m_pDirectionalLightMaterial->SetVariable_Texture(L"gTextureDiffuse", gbufferSRVs[static_cast<int>(DeferredRenderer::eGBufferId::Diffuse)]);
		m_pDirectionalLightMaterial->SetVariable_Texture(L"gTextureSpecular", gbufferSRVs[static_cast<int>(DeferredRenderer::eGBufferId::Specular)]);
		m_pDirectionalLightMaterial->SetVariable_Texture(L"gTextureNormal", gbufferSRVs[static_cast<int>(DeferredRenderer::eGBufferId::Normal)]);
		m_pDirectionalLightMaterial->SetVariable_Texture(L"gTextureDepth", gbufferSRVs[static_cast<int>(DeferredRenderer::eGBufferId::Depth)]);
...
void DeferredLightRenderer::VolumetricLightPass(const SceneContext& sceneContext, ID3D11ShaderResourceView* const gbufferSRVs[], ID3D11RenderTargetView* pDefaultRTV) const
{
	...
	//Ambient SRV > Already on Main RenderTarget
	m_pVolumetricLightMaterial->SetVariable_Texture(L"gTextureDiffuse", gbufferSRVs[static_cast<int>(DeferredRenderer::eGBufferId::Diffuse)]);
	m_pVolumetricLightMaterial->SetVariable_Texture(L"gTextureSpecular", gbufferSRVs[static_cast<int>(DeferredRenderer::eGBufferId::Specular)]);
	m_pVolumetricLightMaterial->SetVariable_Texture(L"gTextureNormal", gbufferSRVs[static_cast<int>(DeferredRenderer::eGBufferId::Normal)]);
	m_pVolumetricLightMaterial->SetVariable_Texture(L"gTextureDepth", gbufferSRVs[static_cast<int>(DeferredRenderer::eGBufferId::Depth)]);
...
void DeferredLightRenderer::CreateReadOnlyDSV(const D3D11Context& d3dContext, ID3D11Resource* pDepthResource, DXGI_FORMAT format)
{
	...
	descDSV.Flags = D3D11_DSV_READ_ONLY_DEPTH;	// Depth read-only (stencil still read/write)
